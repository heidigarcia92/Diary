...
void ActivityList::setName(string _title) {
    _name = _title;
}
...
std::ostream & operator<<(std::ostream &os, ActivityList &al) {
    
    for (Activity& a : al.activities) {
        os << a;
    }
    
    return os << ' ' << al._name << ' ' << al._position << ' ' << al.nextActivityPosition;
}